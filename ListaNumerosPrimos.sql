DECLARE @esPrimo BIT,				-- Lo usaremos como boolean
		@rInicial INT = 2,			-- Donde empieza el bucle, empieza en 2 por que todo número es divisible por la únidad 
		@divPrimos INT = 2,			-- El contador que emezará a dividir por números hasta llegar al @rInicial sin incluirlo
...
SET @esPrimo = 1;					-- (1 = true, 0 = false), es primo hasta q se demuestre lo contrario
	...
	BEGIN
		IF @rInicial % @divPrimos = 0 -- Este while fija el @rInicial y busca si desde 2 (@divPrimos) a @rIncial hay algún nº con resto 0
		BEGIN
			SET @esPrimo = 0	-- Si hay alguna división con resto 0 cambiamos el valor @esPrimo a 0
		END
		SET @divPrimos += 1		-- Aquí vamos aumentando el divisor para comprobar que no hay ninguna división con resto 0
	END
